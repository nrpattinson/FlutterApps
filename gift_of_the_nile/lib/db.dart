...
class GameDatabase {
  ...
  Future<Database> _initializeDatabase() async {
    ...
    final dbPath = path.join(appDocumentsDir.path, 'GiftOfTheNile', 'gift_of_the_nile.db');
  ...
  Future<List<Map<String, dynamic>>> fetchGameCompletedList() async {
    Database db = await _db;
    return await db.rawQuery('''
      SELECT g.gameId, g.scenario, g.optionsJson, g.startTime, g.finishTime, s.stage
  ...
  Future<Map<String, dynamic>> fetchGameCompleted(int gameId) async {
    Database db = await _db;
    final records = await db.rawQuery('''
      SELECT g.scenario, g.optionsJson, g.outcomeJson, g.log, s.stateJson, s.step, s.subStep
      FROM gameCompleted g, gameSnapshot s
  ...
  Future<Map<String, dynamic>> fetchGameSnapshot(int gameId, int sequence) async {
    Database db = await _db;
    final records = await db.rawQuery('''
      SELECT stateJson, step, subStep, stage, logLength
      FROM gameSnapshot
