    ...
    if (localState.subStep == 0) {
      ...
      if (frontArmyCount < 5) {
        ...
        if (overTheTopPossible) {
          ...
          int dice = roll2D6();
          if (dice <= piecesInLocationCount(PieceType.city, Location.unruhe)) {
 ...
 void siegfriedLine() {
  ...
  void navalAirWarfarePhaseDeployCruisers() {
    ...
    if (cruisers.isNotEmpty) {
      ...
      int dice = roll2D6();
      final locations = cruiserTable[dice - 2][cruisers.length - 1];
...
void nearEastPhaseIndiansAttackAskari() {
  ...
  for (int i = 0; i < armies.length; ++i) {
    ...
    int dice = roll2D6();
    if (dice != 12 && _options.indiansTwoRolls) {
      dice = roll2D6();
    }
    if (dice == 12) {
      logLine('> Attack succeeded.');
...
void nearEastPhaseOttomanGroundCombat() {
  ...
  while (true) {
    ...
    if (checkChoice(Choice.yes) || checkChoice(Choice.no)) {
      ...
      if (total <= armyValue(army) && pieceLocation(Piece.kemal) == location) {
        logLine('> Kemal');
        die = rollD6();
        total = die + modifiers;
      }
